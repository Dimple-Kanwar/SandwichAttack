...
import { sendPuissantBundle } from './core/sandwicher-bundle';
...
const Main = async () => {
  ...
  //   }
  // }
  await sendPuissantBundle().then((res)=>{
    console.log("Sent Puissant bundle...");
  }).catch((err) => {
    throw new Error("Error while sending bundled transactions to puissant. Error: " + JSON.stringify(err));
  })
};
