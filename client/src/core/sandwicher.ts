...
class Sandwicher {
  ...
  createSubmarineContract = async (
    ...
    const contractCreateTx = await this.contract.createSubContract(
      buy_salt,
      this.signer.address, overloads
    );

    console.log("Create submarine address tx: ", contractCreateTx);
  ...
  sendTokenstoSubmarine = async (
    ...
    const res = await tokenContract.approve(submarineAddress, amountWei, overloads);
    console.log("approve result: ", res);

    // Send tokens to Submarine contract
    const txSend = await tokenContract.transfer(submarineAddress, amountWei, overloads);
  ...
  revealBuyTransaction = async (
    ...
    // Execute swap
    const revealTx = await this.revealContract.revealBuyExecution(
      salt,
      _data,
      overloads
    );
    console.log("Reveal transaction reference: ", revealTx.hash);
  ...
  // Execute reveal
  const revealTx = await this.revealContract.revealSellExecution(
    sell_salt,
    _data, 
    overloads
  );
